...
## 10. P4 outcome: the L1/L2/L4 split, all five categories

`sections` (3) and `analyticalfields` (3) ported mechanically, following the output-manager
adapter shape (one central adapter in `abqmodelconstructor.py` resolving the same
`material`/`elementSets` names for every module in the category). `planeRandomThickness` and
`mapped.py` were deliberately left as-is (no `.inp` reachability either way, zero test coverage,
not this refactor's decision to make).

`constraints` (12) **broke that uniform-adapter shape**: each module resolves a different set of
structural names to model objects, including a third distinct registry (`model.rigidBodies`), so
one central adapter can't resolve them all the same way. Fix: gave `ConstraintBase`/
`MultiPointConstraintBase` a `fromConstraintDefinition(name, definition, model)` classmethod
mirroring `StepActionBase.fromStepActionDefinition` (P3's L4 seam for the same reason) — each module
resolves its own names, the adapter is one line. A real bug (`dict(aCaseInsensitiveDict)` silently
producing lowercased plain-dict keys, breaking `.pop("nSet")`) was caught only by the full runner
suites, not by unit tests or standalone construction — logged as a standing lesson.

`solvers` (7) needed **no code changes at all**: the registry fold had already made
`getSolverByName` a thin `registry.lookup` wrapper, and both call sites already constructed through
it. The remaining `schema=None` is not a gap to close — `SolverSpecificOptions` +
`NonlinearSolverBase._updateOptions` is a bespoke, working mechanism built for *mutable merge*
semantics (`>>options` blocks re-apply non-strict and partial, every step) that a frozen
`schemaField` dataclass does not naturally support without touching `stepactions/options.py`'s
invariant-guarded `getOptionsOfCategory`. Sanctioned as the same shape of gap step actions already
have, for the identical reason.

`generators` (10) was the one category flagged from the plan's own P4 row as a genuine design call:
registry entries resolved to a bare module-level function (`generateModelData`), not a class, so
`schemaOf` structurally returned `None` for all ten. Put to the user directly (convert to classes /
bolt a `schema` attribute onto the function via a targeted exception to `schemaOf` / leave
`schema=None` permanently like solvers); **decided: convert to classes**, matching every other
category's shape and requiring no special-casing anywhere in the framework. New
`generators/base/generatorbase.py::GeneratorBase` — unlike every other category, a generator has no
interface beyond its constructor (populating `model` *is* what it does, in `__init__` directly), so
`fromGeneratorDefinition` is a classmethod building a schema from `kwargs` and constructing the
class, overridden only by `executePythonCode` (raw code datalines, no flat option mapping, `schema =
None` deliberately). Two real cross-module dependencies surfaced and were fixed during conversion,
not just mechanically re-pointed: `cuboidlatticegenerator.py` called `boxgen.generateModelData`
directly as a plain function (now instantiates `boxgen.Generator`), and the free function
`replicateMesh` (shared by `cuboidlatticegenerator.py`/`microstructuregenerator.py`) took an
`options: dict` parameter read only as `options["elType"]` — narrowed to a plain `elTypeName: str`
parameter, since a frozen schema dataclass is not a dict. `buildContactFacets` and
`generateDiscreteRigidBodyFromMeshFile` stay free functions, since other modules call them directly.

**P4 is complete.** Registry: 13 categories / 126 entries, none uncovered. L1/L2 (typed constructor
+ schema): done for 58 modules across output managers, step actions, sections, analyticalfields,
constraints and generators; deliberately `schema=None` for step actions, solvers and
`executePythonCode`, for the reasons above, not by omission. Every one of those 58 modules still
carries the legacy `Module`/`InputLanguage()` declaration *alongside* its new schema — by design,
per every module's own docstring ("the `Module` one goes away with the `InputLanguage` singleton in
P5") — so P5 is the next phase, not further L1/L2/L4 work.

## Scope of this branch

**Current as of 2026-07-29:** this branch implements **P0 through P4, complete** — including all
five L1/L2/L4 split categories (see §10). P5 (retire the `InputLanguage` singleton) and P6 (share
the L4 seam with EdelweissMeshfree) are untouched and are the natural next phase. See §7 (P1), §8
(P3(c)), §9 (P4's registry-coverage checkpoint) and §10 (P4's L1/L2/L4 split) for the per-phase
outcome records, and the untracked `HANDOFF_INPUT_SYSTEM.md` for the resumption brief: branch stack,
verification baselines, and the deviation-fingerprint technique.
