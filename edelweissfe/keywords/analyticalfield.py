...
class AnalyticalFieldKeyword(KeywordBase):
    ...
    def fromKeywordDefinition(cls, name: str, definition: dict, context: InputContext) -> "KeywordBase | None":
        ...
        ------
        NotImplementedError
            Always. Construction from a parsed ``*analyticalField`` definition is wired in U3, once
            the runtime parser is swapped over (see ``PLAN_INPUT_SYSTEM_UNIFICATION.md``, U3).
        """
        raise NotImplementedError("AnalyticalFieldKeyword.fromKeywordDefinition is wired in U3, not U2b.")
