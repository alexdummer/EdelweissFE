...
cdef class CSRGenerator:
    ...
    def __init__(self, systemMatrix: VIJSystemMatrix):
        ...
        cdef:
            long[::1] IDX = systemMatrix.I
            long[::1] J = systemMatrix.J
            long nDof = systemMatrix.nDof

        self.nDof = nDof
        self.x = np.zeros_like (IDX, dtype=np.intc)

        # abuse Scipy to create the object
        self.csrMatrix = csr_matrix((np.zeros_like(IDX, dtype=np.double), (IDX, J)), (nDof, nDof))
        ...
        cdef long row, col
        self.nCooPairs = len(IDX)
        for cooPairIdx in range(self.nCooPairs):
            row = IDX[cooPairIdx]
            col = J [cooPairIdx]

            # binary search algorithm
            # (can be improved based on lookup tables, but seems to be sufficient for the moment)
